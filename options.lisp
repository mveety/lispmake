...
(defun initialize-vars (&key (apply-prefix nil))
  (let* ((prefix (get-var 'prefix))
    ...
    (if (or (equal (get-var 'bindir) "") apply-prefix)
		(set-var 'bindir bindir))
    (if (or (equal (get-var 'libdir) "") apply-prefix)
		(set-var 'libdir libdir))
    (if (or (equal (get-var 'etcdir) "") apply-prefix)
		(set-var 'etcdir etcdir))
	(if (nilp apply-prefix)
		(progn
...
(defun pl-apply-prefix (args)
  (declare (ignore args))
  (initialize-vars :apply-prefix t))
